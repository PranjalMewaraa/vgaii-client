...
- **Self-hosted booking** — enable it on the dental client (`/settings` → Booking) to test the slot-picker flow described in section 4.2.
...
- [ ] **Header / Query toggle** on each webhook row works.
...
- [ ] `/patients` shows leads with `appointment_booked` or `visited` status, plus orphan appointments (no matched lead) tagged "Direct appointment".
...
- [ ] Click **Book appointment** → slot picker loads inline below the Next-step card. (If self-hosted booking isn't enabled in settings, an amber notice points to Settings.)
- [ ] Pick a slot, confirm → an Appointment is created for this lead's phone.
- [ ] Lead status auto-bumps to `appointment_booked` once the slot is confirmed (unless it was already `visited`/`lost`/`appointment_booked`).
...
File issues with the section number from this document (e.g. "4.2 step 5 — appointment booked but lead status didn't update").
